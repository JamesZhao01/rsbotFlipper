...
//
//    /**
//     * This function serves as a handler function for the api endpoint. It takes in an http request
...
//
//
////        //TODO add implementation for api
...
//     * with a Google OAuth token. This token is validated, and if it is valid, a JWT issued by this server is sent
//     * in response.
//     * @param request the http request
//     * @param response the http response
...
//            return generateErrorJson(new ErrorResponse(EMPTY_TOKEN_ERROR, EMPTY_TOKEN_ERROR_DESCRIPTION));
//        }
//        // build httpRequest to validate with google api
...
//
//                // generate a jwt
//                String token = generateToken(jsonNode.get("email").toString(), config.JwtConfig.ISSUER, config.JwtConfig.SUBJECT, 36000);
...
//
//        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(config.JwtConfig.SECRET_KEY);
